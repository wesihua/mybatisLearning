...
<body>
	<div class="query">
		<form action="<%=path %>/manager/model1/list" method="post">
			&nbsp;&nbsp;
			<c:if test="${prop['model1_id_show'] == '0'}">
				${prop['model1_id']}：<input type="text" name="id"/>&nbsp;&nbsp;
			</c:if>
			${prop['model1_name']}：<input type="text" name="name"/>
			...
			<tbody>
				<c:forEach var="bean" items="${list}" varStatus="index">
					<tr>
						...
						<td>
							<button onclick="editObject('<%=path%>/manager/model1/editModel','${bean.id}')">修改</button>&nbsp;&nbsp;&nbsp;
							<button onclick="deleteObject('<%=path%>/manager/model1/deleteModel','${bean.id}')">删除</button>
						</td>
					</tr>
				</c:forEach>
			</tbody>
